...
vignette: >
  %\VignetteIndexEntry{Jump-starting visualization with dittoViz}
...
data-type aganostic visualization suite. If there is a plot type missing that
you would like to suggest we add, please do so by
...
dittoViz will eventually be available through Bioconductor.  This section will
be updated once that is the case.
...
All dittoViz functions have a `data_frame` input which takes in data.frame-like
object with observations / individual data points in rows, and features of those
observations in columns.

Here, we'll make use of the documentation point `?dittoExampleData` which was
created entirely in order to allow single-line, fast, generation of an example
`data_frame` input:
