...
#' example_pca <- matrix(rnorm(nsamples*2), nsamples)
#'
#' example_df <- data.frame(
#'         conditions = factor(rep(c("condition1", "condition2"), each=nsamples/2)),
#'         timepoint = rep(c("d0", "d3", "d6", "d9"), each = nsamples/4),
#'         SNP = rep(c(rep(TRUE,7),rep(FALSE,8)), nsamples/15),
#'         groups = sample(c("A","B","C","D"), nsamples, TRUE),
#'         score = seq_len(nsamples)/2,
#'         gene1 = log2(rpois(nsamples, 5) +1),
#'         gene2 = log2(rpois(nsamples, 30) +1),
#'         gene3 = log2(rpois(nsamples, 4) +1),
#'         gene4 = log2(rpois(nsamples, 2) +1),
#'         gene5 = log2(rpois(nsamples, 17) +1),
#'         PC1 = example_pca[,1],
#'         PC2 = example_pca[,2],
...
#'         sample = rep(1:12, each = nsamples/12),
#'         category = rep(c("A", "B"), each = nsamples/2),
#'         subcategory = rep(as.character(rep(1:3,4)), each = nsamples/12),
#'         row.names = paste0("sample", 1:nsamples)
#'         )
#'
#' # cleanup
#' rm(example_pca, nsamples)
#'
#' summary(example_df)
NULL
