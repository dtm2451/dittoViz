...
dotPlot <- function(
    ...
    data <- .data_gather_summarize_vars_by_groups(
        data_frame, vars, group.by, split.by,
        summary.fxns = list(summary.fxn.color, summary.fxn.size),
        names = c("color", "size"), rows.use = rows.use,
        var.adjustment = var.adjustment, var.adj.fxn = var.adj.fxn,
        do.hover = do.hover, numeric.only = numeric.only)
...
.data_gather_summarize_vars_by_groups <- function(
    ...
    groupings <- ._col(group.by, data_frame, rows.use = rows.use)

    ### Grab and adjust vars data per observation.  Also grab split.by with no adjustment
    vars_data <- .multi_col_gather_and_adjust(
        data_frame, vars, var.adjustment, var.adj.fxn, split.by, rows.use,
        numeric.only)
    ...
    # rows = individual summary points; each var for group1, group2, group3, ... per each facet
    df_out <- do.call(
        rbind,
        lapply(
            unique(facet),
            function(this_facet) {
                ...
                summary_data <- lapply(
                    summary.fxns, function(f) {
                        .summarize_cols_by_fxn_per_group(
                            f, df = vars_data, groups = groupings, cols = vars)
                    })
    ...
    if (do.hover) {
        df_out$hover.string <- .make_hover_strings_from_df(df_out)
    }
...
### Summarize vars data per group
## Inputs:
# summary.fxn = How to summarize the vars data per group
# df = a data.frame with observations in rows and data features in columns
# groups = a vector that's unique per each 'grouping' -- sets of rows of df that should be put through the summarize function together
# cols = columns of 'df' to summarize per group
## Output: dataframe with summarized cols data (1 row per element of cols)
# rows = 'cols', nrows = length(cols)
# cols = groupings
.summarize_cols_by_fxn_per_group <- function(summary.fxn, df, groups, cols) {
        ...
        vapply(
            ...
            function (this_group) {
                vapply(
                    cols,
                    function(this_var) {
                        summary.fxn(df[groups == this_group, this_var])
                    }, FUN.VALUE = numeric(1)
                )
...
#' @importFrom stats sd
.multi_col_gather_and_adjust <- function(
        data_frame,
        vars,
        var.adjustment,
        var.adj.fxn,
    ...
    df_adj <- data_frame[rows.use, vars]

    # Gather and transform 'vars' data.
    if (numeric.only) {
        # Only check vars
        for (var in vars) {
            if (!is.numeric(df_adj[,var])) {
                stop("All 'vars' must be numeric. ", var, " is not numeric.")
            }
        }
    }

    if (!is.null(var.adj.fxn)) {
        df_adj <- apply(df_adj, 2, var.adj.fxn)
    } else if (!is.null(var.adjustment)) {
        if (var.adjustment=="z-score") {
    ...
    # Add cols not needing adjustment
    cbind(df_adj, data_frame[rows.use, vars.no.adj])
}
